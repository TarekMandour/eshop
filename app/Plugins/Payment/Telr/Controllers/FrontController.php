<?php
#App\Plugins\Payment\Telr\Controllers\FrontController.php
namespace App\Plugins\Payment\Telr\Controllers;

use App\Plugins\Payment\Telr\AppConfig;
use App\Http\Controllers\RootFrontController;
use App\Plugins\Payment\Telr\Lib\CreateOrder;
use App\Plugins\Payment\Telr\Lib\CaptureOrder;
...
class FrontController extends RootFrontController
{
    ...
    public function index() {
        ...
        $dataMapping = [
            'intent' => 'CAPTURE',
            'application_context' => [
                'return_url' => sc_route('telr.return', ['orderId' => $dataPayment['reference_id']]),
        ...
        if (!empty(session('paypal_express_payment_id'))) {
            ...
            if ($captureId) {
                ShopOrder::find($orderId)->update([
                    ...
                    'status' => sc_config('Telr_order_status_success'),
                    'payment_status' => sc_config('Telr_payment_status')
                    ]
                );
                //Add history
                $dataHistory = [
                    ...
                    'order_status_id' => sc_config('Telr_order_status_success'),
                ];
    ...
    public function processOrder() {
        ...
        if ($orderID && $dataOrder && $arrCartDetail) {
            ...
            return redirect()->route('telr.index')->with('dataPayment', $dataPayment);
        } else {
