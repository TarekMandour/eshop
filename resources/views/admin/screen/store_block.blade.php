                ...
                <div class="card-header with-border">
                    ...
                    <div class="card-title">
                        <div class="btn-group float-right" style="margin-right: 5px">
                            <a href="{{ sc_route_admin('admin_store_block.index') }}" class="btn  btn-flat btn-default" title="List"><i class="fa fa-list"></i><span class="hidden-xs"> {{trans('admin.back_list')}}</span></a>
                ...
                <form action="{{ $url_action }}" method="post" accept-charset="UTF-8" class="form-horizontal" id="form-main"  enctype="multipart/form-data">
                    ...
                    <div class="card-body">
                            <div class="form-group row  {{ $errors->has('name') ? ' text-red' : '' }}">
                                <label for="name" class="col-sm-2 col-form-label">{{ trans('store_block.name') }}</label>
                                <div class="col-sm-8">
                            ...
                            <div class="form-group row {{ $errors->has('position') ? ' text-red' : '' }}">
                                <label for="position" class="col-sm-2 col-form-label">{{ trans('store_block.admin.select_position') }}</label>
                                <div class="col-sm-8">
                            ...
                            <div class="form-group row {{ $errors->has('page') ? ' text-red' : '' }}">
                                <label for="page" class="col-sm-2 col-form-label">{{ trans('store_block.admin.select_page') }}</label>
                                <div class="col-sm-8">
                            ...
                            <div class="form-group row {{ $errors->has('type') ? ' text-red' : '' }}">
                                <label for="type" class="col-sm-2 col-form-label">{{ trans('store_block.type') }}</label>
                                <div class="col-sm-8">
                            @if ($layout)
                            ...
                            <div class="form-group row {{ $errors->has('text') ? ' text-red' : '' }}">
                                <label for="text" class="col-sm-2 col-form-label">{{ trans('store_block.text') }}</label>
                                <div class="col-sm-8">
                                    @php
                                    ...
                                    <span class="form-text"><i class="fa fa-info-circle"></i> {{ trans('store_block.admin.helper_html') }}</span>
                                    @elseif ($dataType =='view')
                                        ...
                                        <span class="form-text"><i class="fa fa-info-circle"></i> {{ trans('store_block.admin.helper_view',['template' => sc_store('template', session('adminStoreId'))]) }}</span>
                                    @else
                                        <textarea name="text" class="form-control text" rows="5" placeholder="Layout text">
                                            {!! old('text',$layout['text']??'') !!}
                                        </textarea>
                            ...
                            <div class="form-group row  {{ $errors->has('sort') ? ' text-red' : '' }}">
                                <label for="sort" class="col-sm-2 col-form-label">{{ trans('store_block.sort') }}</label>
                                <div class="col-sm-8">
                                    <div class="input-group">
                                        <div class="input-group-prepend">
                                        <span class="input-group-text"><i class="fas fa-pencil-alt"></i></span>
                            ...
                            <div class="form-group row ">
                                <label for="status" class="col-sm-2 col-form-label">{{ trans('store_block.status') }}</label>
                                <div class="col-sm-8">
...
$(function () {
    ...
    if(type =='html'){
       obj.before('<textarea name="text" class="form-control text" rows="5" placeholder="Layout text"></textarea><span class="form-text"><i class="fa fa-info-circle"></i> {{ trans('store_block.admin.helper_html') }}.</span>');
       obj.remove();
    }else if(type =='view'){
       obj.before('<select name="text" class="form-control text">@foreach ($listViewBlock as $view)<option value="{{ $view }}">{{ $view }}</option>@endforeach</select><span class="form-text"><i class="fa fa-info-circle"></i> {{ trans('store_block.admin.helper_view',['template' => sc_store('template', session('adminStoreId'))]) }}</span>');
       obj.remove();
    }
    });
});
